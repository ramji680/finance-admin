...
router.get('/stats', (0, errorHandler_1.asyncHandler)(async (_req, res) => {
    try {
        ...
        const [usersResult] = await connection_1.sequelize.query('SELECT COUNT(*) as count FROM users WHERE status = "active"');
        const totalUsers = usersResult[0].count;
        console.log('Users result:', totalUsers);
        // Get total restaurants
        console.log('Testing restaurants query...');
        const [restaurantsResult] = await connection_1.sequelize.query('SELECT COUNT(*) as count FROM restaurant_information WHERE approve_status = "active"');
        const totalRestaurants = restaurantsResult[0].count;
        console.log('Restaurants result:', totalRestaurants);
        // Get total orders
        console.log('Testing orders query...');
        const [ordersResult] = await connection_1.sequelize.query('SELECT COUNT(*) as count FROM orders');
...
router.get('/analytics', (0, errorHandler_1.asyncHandler)(async (_req, res) => {
    try {
        ...
        for (let month = 5; month >= 0; month--) {
            ...
            const [monthResult] = await connection_1.sequelize.query(`
        SELECT 
      ...
      `, {
            ...
            const monthRevenue = parseFloat(monthResult[0].totalRevenue || '0');
            const monthCommission = parseFloat(monthResult[0].totalCommission || '0');
      ...
      `, {
            ...
            const monthRevenue = parseFloat(monthResult[0].totalRevenue || '0');
            const monthOrderCount = parseInt(monthResult[0].orderCount || '0');
      ...
      LIMIT 5
    `);
        const topRestaurants = topRestaurantsResult.map(restaurant => ({
...
router.get('/recent-activity', (0, errorHandler_1.asyncHandler)(async (_req, res) => {
    try {
        ...
        const [recentOrdersResult] = await connection_1.sequelize.query(`
      SELECT 
        o.id,
        o.order_id,
        r.restaurant_name,
      ...
      LIMIT 10
    `);
        const [recentPaymentsResult] = await connection_1.sequelize.query(`
      SELECT 
        id,
        order_id,
        amount,
      ...
      LIMIT 5
    `);
        const [recentTicketsResult] = await connection_1.sequelize.query(`
      SELECT 
        id,
        subject,
        status,
    ...
    `);
        const recentActivity = {
            ...
            recentPayments: recentPaymentsResult.map(payment => ({
                id: payment.id,
            ...
            recentSupportTickets: recentTicketsResult.map(ticket => ({
                id: ticket.id,
